...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    let wavesStuff = Bundle.main.decode([Wave.WaveEnemy].self, from: "enemycharacteristics.json")
    ...
    func createWave() {
        ...
        if ((currentWave?.enemies.isEmpty) != nil) {
            for enemy in enemyTypes! {
                for (index, oponent) in Oponents.shuffled().enumerated() {

                
                    let enemy = EnemyNode(type: wavesStuff![oponent], enemyType: enemyTypes![enemyType], names: "", position: CGPoint(x: enemyStartX, y: positions[oponent]), xOffset: enemyOffsetX * CGFloat(index * 3), moveStraight: true)
                    addChild(enemy)
                }
            }
        } else {
...
//                for (_, oponent) in Oponents.shuffled().enumerated() {
                for (index, oponent) in Oponents.shuffled().enumerated() {
                    ...
                    let node = EnemyNode(type: wavesStuff![oponent], enemyType: enemyTypes![enemyType], names: "", position: CGPoint(x: enemyStartX, y: positions[oponent]), xOffset: enemyOffsetX * CGFloat(index * 3), moveStraight: true)
